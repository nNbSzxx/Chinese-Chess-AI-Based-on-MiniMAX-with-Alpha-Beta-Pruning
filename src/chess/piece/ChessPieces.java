...
//	public final int value;
	...
	public ChessPieces(int iniX, int iniY, boolean iniIsRed, boolean iniIsAlive, String iniName) {
...
//	abstract public boolean isCheck(ChessBoard board);
	...
	private int pieceInStraightVerticalWay(ChessBoard board, int toX, int toY) {
		...
		if (toX > this.x) {
			...
			// toX < this.getX
			for (int i = x - 1; i > toX; i --) {	//这里把原来的  '<' 改成   '>'
				if (board.getPiece(i, y) != null) {
					++ cnt;
				}
			}
...
public abstract class ChessPieces {
	...
	public ChessPieces(int iniX, int iniY, int iniValue, boolean iniIsRed, boolean iniIsAlive) {
		// TODO Auto-generated constructor stub
		x = iniX;
		y = iniY;
		value = iniValue;
